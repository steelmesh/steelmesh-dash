var async = require('async'),
    ...
    reVersion = /^.*(?:\/|\\)(.*?)\-([\d\.\-]+)\..*$/,
    reAppFile = /app\.js/,
    assetsPath = path.resolve(__dirname, '../../../_deploy'),
    packagesPath = path.join(assetsPath, 'packages'),
...
exports.connect = function(server, steelmesh, dash, callback) {
    if (dash.mode === 'primary') {
        ...
        server.get('/deploy/:appid', _makePublisher(dash.config, dash));
